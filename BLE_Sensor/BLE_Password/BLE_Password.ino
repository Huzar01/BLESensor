...
startMotor(characteristic.value(), characteristic.valueLength());
}

void startMotor(const unsigned char* code, int codeLength) {
...
//            Serial.println(code);
  ...
  if (match) {
    // turn on motor
    Serial.println("Code matches, sensor is now active");
    digitalWrite(MOTOR_PIN, HIGH);
